...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      StructField(columnNames.head, StringType, nullable = false) :: columnNames.tail.map(
        s => StructField(s, DoubleType, nullable = false)))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def spreadType(columnName: String): (String, String) = {
      val primary = """((?:t01|t03|t11|t1801|t1803).+)""".r
      val work = """((?:t05|t1805).+)""".r
      val other = """((?:t02|t04|t06|t07|t08|t09|t10|t12|t13|t14|t15|t16|t18).+)""".r
      columnName match {
        case primary(name) => ("primary", name)
        case work(name) => ("work", name)
        case other(name) => ("other", name)
        case _ => ("ignore", null)
      }
    }

    val columns = columnNames.map(spreadType)
    (
      columns.filter(_._1 == "primary").map(pair => col(pair._2)),
      columns.filter(_._1 == "work").map(pair => col(pair._2)),
      columns.filter(_._1 == "other").map(pair => col(pair._2))
    )
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working, row.sex, row.age)).agg(
